...
/**
 * @brief 	Fonction de démonstration permettant d'apr�hender l'utilisation de ce module logiciel
 * @pre		Cette fonction doit être appelée en tâche de fond
 * @pre		Il faut avoir préalablement initialisé un port s�rie (cette fonction utilise printf)
...
HAL_StatusTypeDef BSP_HCSR04_delete(uint8_t id)
{
	HAL_StatusTypeDef ret;
	ret = HAL_ERROR;
	if(id < HCSR04_NB_SENSORS)
	{	
		if(sensors[id].state != HCSR04_STATE_INEXISTANT)
		{
			
			BSP_GPIO_pin_config(sensors[id].echo_gpio, sensors[id].echo_pin, GPIO_MODE_INPUT, GPIO_NOPULL, GPIO_SPEED_FREQ_HIGH, GPIO_NO_AF);
			BSP_GPIO_pin_config(sensors[id].trig_gpio, sensors[id].trig_pin, GPIO_MODE_INPUT, GPIO_NOPULL, GPIO_SPEED_FREQ_HIGH, GPIO_NO_AF);
			BSP_EXTIT_disable(BSP_EXTIT_gpiopin_to_pin_number(sensors[id].echo_pin));
			sensors[id].state = HCSR04_STATE_INEXISTANT;
			ret = HAL_OK;
		}
	}
	return ret;
