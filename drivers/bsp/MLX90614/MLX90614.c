 ...
 *
 * Par défaut, les broches utilisées sont celles de l'I2C1 : PB7 pour SDA et PA15 pour SCL.
...
{
	BSP_I2C_Init(MLX90614_I2C, STANDARD_MODE, true);
}


/*
 * Machine à états du capteur de temérature
 ...
 */
running_t DEMO_MLX90614_statemachine(bool ask_for_finish)
{
	typedef enum
	{
		INIT = 0,
	...
	switch(state)
	{
		case INIT:
			MLX90614_init();
			BSP_systick_add_callback_function(DEMO_MLX90614_process_1ms);
			state = RUN;
			break;
		case RUN:
			if(ask_for_finish)
		...
		case CLOSE:
			state = INIT;
			BSP_systick_remove_callback_function(DEMO_MLX90614_process_1ms);
			ret = END_OK;
			break;
	}
...
{
  uint8_t ret[3];
  BSP_I2C_ReadMulti(MLX90614_I2C, MLX90614_I2CADDR<<1, a, ret, 3);
